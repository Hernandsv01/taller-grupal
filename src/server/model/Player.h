   ...
   public:
    Player(int id, float x_spawn, float y_spawn)
        : Dynamic_entity(id, x_spawn, y_spawn, PLAYER_WIDTH, PLAYER_HEIGHT,
                         ...
                         true, 0, false, PLAYER_HEALTH, true, true),
          points(0),
        ...
        switch (action) {
            ...
            case SHOOT:
                ...
                entity_pool.push_back(
                    std::make_unique<Bullet>(next_id, x_spawn, y_spawn, speed));
    ...
    void delete_pickup(
        std::vector<std::unique_ptr<Dynamic_entity>>& entity_pool, int id) {
        auto it =
            std::find_if(entity_pool.begin(), entity_pool.end(),
